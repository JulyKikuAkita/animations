...
struct SwipeAction<Content: View>: View {
    ...
    var body: some View {
        ScrollViewReader { scrollProxy in /// reset scroll view to original position  when swipe action is pressed
            ...
            .scrollIndicators(.hidden)
            // (where the magic is) ViewAligned scroll target behavior
            // requires scrollTargetLayout() to be added inside the scrollView
                    ...
                    ForEach(filterActions) { button in
                        Button(action: {
                            Task {
                                isEnabled = false
                                resetPosition()
                                // scroll action approx. takes 0.25 secs to complete
                                // (optionally if want immediate run button action
