                ...
                .overlay {
                    if useKeyFrame {
                        ForEach(buttonFrames2) { frame in
                            KeyframeAnimator(
                                initialValue: ButtonFrame(value: 0),
                                trigger: frame.triggerKeyFrame
                            ) { frame in
    ...
    func toggleAnimation(_ id: UUID, _: Bool = true) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.01) { // we need the delay for keyFrame animation to show up
            if let index = buttonFrames2.firstIndex(where: { $0.id == id }) {
                ...
                /// keyframe animation takes about 0.8,
                /// use the same delay to remove frame to decrease memory usage
...
//                                Image(systemName: imageName)
    ...
    func removeFrame(_ id: UUID) {
        /// keyframe animation takes about 0.8, use the same delay to remove frame to decrease memory usage
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
